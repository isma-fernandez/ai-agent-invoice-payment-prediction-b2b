...
@mcp.tool()
async def search_client(name: str, limit: int = 5):
...
@mcp.tool()
async def get_client_info(partner_id: int):
...
async def get_client_invoices(partner_id: int, limit: int = 5, only_unpaid: bool = False, paid_only: bool = False):
    ...
    """
    dm = await get_dm()
    return await dm.get_client_invoices(
...
@mcp.tool()
async def get_invoice_by_name(invoice_name: str):
...
@mcp.tool()
async def get_overdue_invoices(limit: int = None, min_days_overdue: int = 1):
...
@mcp.tool()
async def get_upcoming_due_invoices(days_ahead: int = 7, limit: int = 20):
...
@mcp.tool()
async def get_invoices_by_period(start_date: str, end_date: str, partner_id: int = None, only_unpaid: bool = False):
